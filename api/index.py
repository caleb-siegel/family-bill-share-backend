...
def check_auth():
    ...
    try:
        ...
        is_configured = family_count > 0 and email_count > 0
...
def get_families():
    ...
    try:
        ...
        cur = conn.cursor()
        cur.execute("""
            SELECT id, family, created_at, updated_at
            FROM group_bill_automation.bill_automator_families 
            WHERE user_id = %s
...
def create_family():
    ...
    try:
        ...
        cur.execute("""
            ...
            VALUES (%s, %s, NOW(), NOW())
            RETURNING id, family, created_at
        """, (request.user_id, data['name']))
        ...
        return jsonify({
            ...
            "family": {
                ...
                "created_at": family_data[2].isoformat() if family_data[2] else None
            }
        }), 201
...
def get_emails():
    ...
    try:
        ...
        cur = conn.cursor()
        cur.execute("""
            SELECT id, emails, created_at, updated_at
            FROM group_bill_automation.bill_automator_emails 
            WHERE user_id = %s
            ORDER BY created_at DESC
        """, (request.user_id,))
...
@require_auth
def create_email():
    """Add a new email for the authenticated user."""
    try:
        data = request.get_json()
        ...
        return jsonify({
            ...
            "email": {
                "id": email_data[0],
                "emails": email_data[1],
                "created_at": email_data[2].isoformat() if email_data[2] else None
            }
        }), 201
